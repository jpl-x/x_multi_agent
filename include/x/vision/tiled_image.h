...
class TiledImage : public cv::Mat {
 public:
  ...
  explicit TiledImage(const Mat &cv_img);

  TiledImage(const TiledImage &img);

  TiledImage(const Mat &img, const double &timestamp, unsigned int frame_number,
  ...
  void setTileSize(double tile_height, double tile_width);

  void setTileParams(unsigned int n_tiles_h, unsigned int n_tiles_w,
                     unsigned int max_feat_per_tile);
  ...
  [[nodiscard]] double getTileHeight() const;

  [[nodiscard]] double getTileWidth() const;

  [[nodiscard]] unsigned int getNTilesH() const;

  [[nodiscard]] unsigned int getNTilesW() const;

  [[nodiscard]] unsigned int getMaxFeatPerTile() const;

  // Get the feature count for a specific tile
  [[nodiscard]] unsigned int getFeatureCountAtTile(unsigned int row,
  ...
  void plotFeature(const Feature &feature, const cv::Scalar &color);
 ...
 private:
  ...
  unsigned int max_feat_per_tile_ =
      40;  // maximum count of features per image tile
  unsigned int n_tiles_ =
      1;  // total number of image tiles (n_tiles_w_ * n_tiles_h)
  std::vector<unsigned int>
      tiles_;  // n_tiles_w_ x n_tiles_h 1D array that holds count

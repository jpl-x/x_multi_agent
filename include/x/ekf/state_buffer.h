...
class StateBuffer : public std::vector<State> {
 public:
   ...
   *                        range.
   */
  explicit StateBuffer(const size_t size = 250,
                       const State &default_state = State(),
                       const double time_margin = 0.005)
      : std::vector<State>(size, default_state), time_margin_{time_margin} {}

  /**
   * Get index the most recent IMU state in the buffer (tail).
   */
  [[nodiscard]] int getTailIdx() const { return tail_idx_; };
   ...
   */
  [[nodiscard]] int closestIdx(const double timestamp) const;
   ...
   */
  [[nodiscard]] int nextIdx(int idx) const;
   ...
   */
  [[nodiscard]] int previousIdx(int idx) const;

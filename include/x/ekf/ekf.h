...
class Ekf {
 public:
   ...
   *                            time range.
   */
  void set(const Updater &updater, const Vector3 &g, const ImuNoise &imu_noise,
           const int state_buffer_sz, const State &default_state, double a_m_max,
           unsigned int delta_seq_imu, const double &time_margin_bfr);
   ...
   * EKF update function.
   */
  std::optional<State> processUpdateMeasurement();

#ifdef MULTI_UAV
  /**
   * Process the SLAM measurements from other agents
   *
   * @param timestamp
   * @return
   */
  std::optional<State> processOthersMeasurement(double timestamp);
#endif
 ...
 private:
   ...
   */
  bool repropagateFromStateAtIdx(const State &state, int idx);
};
 ...
 */
class init_bfr_mismatch : std::exception {};
}  // namespace x

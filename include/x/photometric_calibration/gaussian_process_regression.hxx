...
template <typename R>
GaussianProcessRegression<R>::GaussianProcessRegression(int inputDim,
                                                        int outputDim) {
...
template <typename R>
void GaussianProcessRegression<R>::AddTrainingData(const VectorXr& newInput,
                                                   const VectorXr& newOutput) {
  n_data_++;
...
template <typename R>
void GaussianProcessRegression<R>::AddTrainingDataBatch(
    const MatrixXr& newInput, const MatrixXr& newOutput) {
  ...
  // if we already have data, first check dimensionaly match
    ...
    input_data_.block(0, n_data_old, newInput.rows(), newInput.cols()) =
        newInput;
    output_data_.block(0, n_data_old, newOutput.rows(), newOutput.cols()) =
        newOutput;
  }
  // in any case after adding a batch of data we need to recompute decomposition
  // (in lieu of matrix inversion)
  b_need_prepare_ = true;
}
...
template <typename R>
typename GaussianProcessRegression<R>::VectorXr
GaussianProcessRegression<R>::SQEcovFunc(MatrixXr x1, VectorXr x2) {
  int nCol = x1.cols();
  VectorXr KXx_local(nCol);
  for (int i = 0; i < nCol; i++) {
    KXx_local(i) = SQEcovFuncD(x1.col(i), x2);
  }
  return KXx_local;
}
...
void GaussianProcessRegression<R>::PrepareRegression(bool force_prepare) {
  ...
  for (int i = 0; i < KXX.cols(); i++) KXX_(i, i) += sigma_n_ * sigma_n_;
  ...
  for (size_t i = 0; i < output_data_.rows(); ++i) {
    alpha_.row(i) =
        (decomposition.solve(output_data_.row(i).transpose())).transpose();
  }
...
// This is the right way to do it but this code should be refactored and tweaked
// so that the decompositon is not recomputed unless new training data has
// arrived.
template <typename R>
typename GaussianProcessRegression<R>::VectorXr
GaussianProcessRegression<R>::DoRegression(const VectorXr& inp, bool prepare) {
...
template <typename R>
typename GaussianProcessRegression<R>::VectorXr
GaussianProcessRegression<R>::DoRegressionOld(const VectorXr& inp,
                                              bool prepare) {
  if (prepare || b_need_prepare_) {
...
template <typename R>
typename GaussianProcessRegression<R>::MatrixXr
GaussianProcessRegression<R>::SQEcovFunc(MatrixXr x1) {
  int nCol = x1.cols();
...
void GaussianProcessRegression<R>::Debug() {
  BOOST_LOG_TRIVIAL(debug) << "input data \n" << input_data_ << std::endl;

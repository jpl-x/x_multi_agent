...
class StateManager {
 public:
  ...
  void initMsckfSlamFeatures(State& state,
                             ...
                             const Matrix& correction, const double sigma_img);
   ...
   *                        depth [1/m].
   */
  void initStandardSlamFeatures(State& state, const Matrix& feature,
  ...
  /// The value of poseSize() is the number of poses stored in the most recent
  /// state. \return The current number of poses.
   ...
   * @param[in] max_size Max size of output list, cropping from the end (default
   * 0: no cropping)
 ...
 private:
  ...
  static unsigned constexpr jac_rows =
      3;  ///< Rows of the Jacobian matrix entries
  static unsigned constexpr jac_cols =
      3;  ///< Cols of the Jacobian matrix entries
   ...
   */
  void augmentCovariance(const State& state, const int pos, Matrix& covariance);
  ...
  void reparametrizeFeatures(Eigen::VectorXd const& atts_old,
                             ...
                             Eigen::VectorXd& features, Matrix& covariance);
  ...
  void slideWindow(Eigen::VectorXd& atts, Eigen::VectorXd& poss,
                   Matrix& covariance);
   ...
   */
  void addFeatureStates(State& state_ptr, const Matrix& new_features,

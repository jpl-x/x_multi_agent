...
Propagator::Propagator(const Vector3 g, const ImuNoise &imu_noise)
    : g_{g}, imu_noise_{imu_noise} {}

void Propagator::set(const Vector3 &g, const ImuNoise &imu_noise) {
  g_ = g;
...
void Propagator::propagateState(const State &state_0, State &state_1) {
  ...
  const auto dt =state_1.time_ - state_0.time_;
  ...
  const Vector3 dv = (state_1.q_.toRotationMatrix() * e_a_1 +
                      state_0.q_.toRotationMatrix() * e_a_0) /
                     2.0;
...
void Propagator::propagateCovariance(const State &state_0, State &state_1) {
  ...
  const CoreCovMatrix f_d =
      discreteStateTransition(dt, state_0, state_1, e_w_1, e_a_1, state_1.q_);

  // Construct the discrete process noise covariance matrix
  const CoreCovMatrix q_d =
      discreteProcessNoiseCov(dt, state_1.q_, e_w_1, e_a_1, imu_noise_.n_w,
                              imu_noise_.n_bw, imu_noise_.n_a, imu_noise_.n_ba);
...
CoreCovMatrix Propagator::discreteStateTransition(
    const double dt, const State &state_0, const State &state_1,
    const Vector3 &e_w, const Vector3 &e_a, const Quaternion &q) const {
  ...
  const Matrix3 a =
      c_q_a_x * (-dt_2_f2 * eye3 + dt_3_f3 * w_x - dt_4_f4 * w_x * w_x);
  const Matrix3 b =
      c_q_a_x * (dt_3_f3 * eye3 - dt_4_f4 * w_x + dt_5_f5 * w_x * w_x);
  const Matrix3 d = -a;
  ...
  CoreCovMatrix f_d = CoreCovMatrix::Identity();
...
//  // avoid filter observability issue. applying observability constraint
//  // http://seanbow.com/papers/tr_2012_1.pdf
//  Matrix phi = f_d;  //.exp(); // probably the exp is already done...
//
//  // Modify the transition matrix
//  Matrix3 R_kk_1 = state_0.q_.normalized().toRotationMatrix();
//  phi.block<3, 3>(kIdxQ, kIdxQ) =
//      state_1.q_.normalized().toRotationMatrix().transpose() * R_kk_1;
//
//  Vector3 u = R_kk_1 * g_;
//  Eigen::RowVector3d s = (u.transpose() * u).inverse() * u.transpose();
//
//  Matrix3 A1 = phi.block<3, 3>(kIdxV, kIdxQ);
//  Vector3 vec = state_0.v_ - state_1.v_;
//  Vector3 w1 = x::Skew(vec(0), vec(1), vec(2)).matrix * g_;
//  phi.block<3, 3>(kIdxV, kIdxQ) = A1 - (A1 * u - w1) * s;
//
//  Matrix3 A2 = phi.block<3, 3>(kIdxP, kIdxQ);
//  vec = dt * state_0.v_ + state_0.p_ - state_1.p_;
//  Vector3 w2 = x::Skew(vec(0), vec(1), vec(2)).matrix * g_;
//
//  phi.block<3, 3>(kIdxP, kIdxQ) = A2 - (A2 * u - w2) * s;

  //   return phi;
  return f_d;
}

void Propagator::propagateCovarianceMatrices(const Eigen::MatrixXd &cov_0,
                                             const CoreCovMatrix &f_d,
                                             const CoreCovMatrix &q_d,
                                             Eigen::MatrixXd &cov_1) {
  // References to covariance blocks (no memory copy).
  // Naming based on Eq. 2.15 in xVIO tech report.
  const auto n = static_cast<int>(cov_0.rows());
  assert(n > kSizeClone - 1);
  const int n_v = n - kSizeClone;  // number of non-core states

  const Eigen::Ref<const CoreCovMatrix> cov_0_ii =
      cov_0.block<kSizeClone, kSizeClone>(0, 0);
  const Eigen::Ref<const Eigen::MatrixXd> cov_0_iv =
      cov_0.topRightCorner(kSizeClone, n_v);
  const Eigen::Ref<const Eigen::MatrixXd> cov_0_vi =
      cov_0.bottomLeftCorner(n_v, kSizeClone);
  const Eigen::Ref<const Eigen::MatrixXd> cov_0_vv =
      cov_0.bottomRightCorner(n_v, n_v);

  Eigen::Ref<CoreCovMatrix> cov_1_ii =
      cov_1.block<kSizeClone, kSizeClone>(0, 0);
  Eigen::Ref<Eigen::MatrixXd> cov_1_iv =
      cov_1.topRightCorner(kSizeClone, n_v);
  Eigen::Ref<Eigen::MatrixXd> cov_1_vi =
      cov_1.bottomLeftCorner(n_v, kSizeClone);
  Eigen::Ref<Eigen::MatrixXd> cov_1_vv = cov_1.bottomRightCorner(n_v, n_v);

  // Covariance block propagation
  // (eq. 2.32-33 in xVIO tech report and eq. 22 in Weiss et al.)
...
CoreCovMatrix Propagator::discreteProcessNoiseCov(
    const double dt, const Quaternion &q, const Vector3 &e_w,
    const Vector3 &e_a, const double n_w, const double n_bw, const double n_a,
  ...
  const double t419 = t416 + t417 + t418 -
                      dt * t343 * t365 * t371 * (1.0 / 6.0) -
  ...
  const double t462 = t456 + t457 + t460 -
                      dt * t343 * t371 * t458 * (1.0 / 6.0) -
  ...
  const double t510 = t504 + t505 + t508 -
                      dt * t343 * t373 * t506 * (1.0 / 6.0) -
  ...
  const double t580 =
      t579 - t374 * t578 - dt * t343 * t373 * t397 * t432 * (1.0 / 6.0);
  ...
  q_d(kIdxP + 0, kIdxP + 0) = dt * t343 * t347 * t361 * (1.0 / 3.0) +
                              dt * t343 * t354 * t359 * (1.0 / 3.0) +
                              dt * t343 * t355 * t360 * (1.0 / 3.0);
  q_d(kIdxP + 0, kIdxP + 1) =
      t375 - dt * t343 * t345 * t355 * (t358 - q3 * q4 * 2.0) * (1.0 / 3.0) -
  ...
  q_d(kIdxP + 0, kIdxBa + 0) = dt * t343 * t346 * t398 * (-1.0 / 6.0);
  q_d(kIdxP + 0, kIdxBa + 1) = t660;
  q_d(kIdxP + 0, kIdxBa + 2) = dt * t343 * t345 * t400 * (-1.0 / 6.0);
  q_d(kIdxP + 1, kIdxP + 0) = t375 -
                              dt * t343 * t344 * t354 * t367 * (1.0 / 3.0) -
                              dt * t343 * t345 * t355 * t376 * (1.0 / 3.0);
  q_d(kIdxP + 1, kIdxP + 1) = dt * t343 * t347 * t378 * (1.0 / 3.0) +
  ...
  q_d(kIdxP + 1, kIdxQ + 0) = dt * t343 * t374 * t384 * (-1.0 / 6.0);
  q_d(kIdxP + 1, kIdxQ + 1) = t581;
  q_d(kIdxP + 1, kIdxQ + 2) = t616;
  q_d(kIdxP + 1, kIdxBa + 0) = dt * t343 * t366 * t398 * (-1.0 / 6.0);
  q_d(kIdxP + 1, kIdxBa + 1) = dt * t343 * t367 * t399 * (-1.0 / 6.0);
  q_d(kIdxP + 1, kIdxBa + 2) = t662;
  q_d(kIdxP + 2, kIdxP + 0) = t402;
  q_d(kIdxP + 2, kIdxP + 1) = t404;
  q_d(kIdxP + 2, kIdxP + 2) = dt * t343 * t347 * t413 * (1.0 / 3.0) +
  ...
  q_d(kIdxP + 2, kIdxQ + 1) = dt * t343 * t373 * t397 * (-1.0 / 6.0);
  q_d(kIdxP + 2, kIdxQ + 2) = t617;
  q_d(kIdxP + 2, kIdxBa + 0) = t658;
  q_d(kIdxP + 2, kIdxBa + 1) = dt * t343 * t372 * t399 * (-1.0 / 6.0);
  q_d(kIdxP + 2, kIdxBa + 2) = dt * t343 * t370 * t400 * (-1.0 / 6.0);
  q_d(kIdxV + 0, kIdxP + 0) = t419;
  q_d(kIdxV + 0, kIdxP + 1) = t420;
  q_d(kIdxV + 0, kIdxP + 2) = t408;
  q_d(kIdxV + 0, kIdxV + 0) =
      t374 * (t428 + t343 * t362 * t425 +
      ...
      t371 * (t428 + t434 +
              dt * t343 *
                  ...
                  (1.0 / 3.0)) +
      dt * t347 * t361 + dt * t354 * t359 + dt * t355 * t360 +
      dt * t343 * t359 * t399 * (1.0 / 3.0) +
      dt * t343 * t361 * t398 * (1.0 / 3.0) +
      dt * t343 * t360 * t400 * (1.0 / 3.0);
  q_d(kIdxV + 0, kIdxV + 1) = t475 + t476 - dt * t344 * t354 * t367 -
                              dt * t345 * t355 * t376 -
  ...
  q_d(kIdxV + 0, kIdxV + 2) =
      ...
      t373 * (t440 + t515 -
              dt * t343 *
                  ...
                  (1.0 / 3.0)) -
      dt * t346 * t347 * t369 - dt * t344 * t354 * t372 -
      dt * t343 * t346 * t369 * t398 * (1.0 / 3.0) -
  ...
  q_d(kIdxV + 0, kIdxQ + 2) =
      -t371 * (t451 + t452 -
               dt * t343 * (t442 + t582 - e_w2 * e_w3 * t362 * (1.0 / 2.0)) *
                   (1.0 / 3.0)) -
      t374 * t622 + t373 * (t452 - dt * t343 * t585 * (1.0 / 3.0));
  q_d(kIdxV + 0, kIdxBw + 0) = dt * t343 * t362 * t502 * (-1.0 / 6.0);
  q_d(kIdxV + 0, kIdxBw + 2) = dt * t343 * t362 * t503 * (-1.0 / 6.0);
  q_d(kIdxV + 0, kIdxBa + 0) = t343 * t346 * t398 * (-1.0 / 2.0);
  q_d(kIdxV + 0, kIdxBa + 1) = t661;
  q_d(kIdxV + 0, kIdxBa + 2) = t343 * t345 * t400 * (-1.0 / 2.0);
  ...
  q_d(kIdxV + 1, kIdxV + 0) =
      t475 + t476 -
      t374 * (t343 * (t384 * t425 - t362 * t473) * (1.0 / 2.0) -
              dt * t343 *
                  (t362 * t483 * (1.0 / 2.0) - t384 * t474 * (1.0 / 2.0) +
                   t425 * t473) *
                  (1.0 / 3.0) +
              dt * t362 * t384) +
      t371 * (t470 + dt * t362 * t381 +
              dt * t343 *
                  (t362 * t485 * (1.0 / 2.0) - t381 * t466 * (1.0 / 2.0) +
                   t362 * t388 * t429) *
                  (1.0 / 3.0)) +
      t373 * (-t470 + dt * t362 * t388 +
              dt * t343 *
  ...
  q_d(kIdxV + 1, kIdxV + 1) =
      ...
      t371 *
          (dt * t458 + dt * t343 * (t381 * t485 + t429 * t461) * (1.0 / 3.0) +
           e_w1 * t343 * t381 * t388) +
      dt * t347 * t378 + dt * t355 * t379 + dt * t354 * t385 +
      dt * t343 * t378 * t398 * (1.0 / 3.0) +
      dt * t343 * t385 * t399 * (1.0 / 3.0) +
      dt * t343 * t379 * t400 * (1.0 / 3.0);
  q_d(kIdxV + 1, kIdxV + 2) = t560;
  q_d(kIdxV + 1, kIdxQ + 0) =
      -t374 * (-dt * t384 + t343 * t473 * (1.0 / 2.0) +
               dt * t343 *
                   (t471 + e_w1 * e_w2 * t388 * (1.0 / 2.0) +
                    e_w1 * e_w3 * t381 * (1.0 / 2.0)) *
                   (1.0 / 3.0)) +
  ...
  q_d(kIdxV + 1, kIdxBw + 0) = dt * t343 * t384 * t502 * (1.0 / 6.0);
  q_d(kIdxV + 1, kIdxBw + 1) = dt * t343 * t388 * t563 * (-1.0 / 6.0);
  q_d(kIdxV + 1, kIdxBw + 2) = dt * t343 * t381 * t503 * (-1.0 / 6.0);
  q_d(kIdxV + 1, kIdxBa + 0) = t343 * t366 * t398 * (-1.0 / 2.0);
  q_d(kIdxV + 1, kIdxBa + 1) = t343 * t367 * t399 * (-1.0 / 2.0);
  ...
  q_d(kIdxV + 2, kIdxV + 2) =
      ...
      t374 *
          (dt * t507 - dt * t343 * (t390 * t497 - t447 * t447) * (1.0 / 3.0) +
           t343 * t390 * t447) +
      dt * t347 * t413 + dt * t354 * t414 + dt * t355 * t415 +
      dt * t343 * t398 * t413 * (1.0 / 3.0) +
  ...
  q_d(kIdxV + 2, kIdxQ + 1) =
      ...
      t373 *
          (t602 + t610 -
           dt * t343 * (t490 + t601 - t390 * t432 * (1.0 / 2.0)) * (1.0 / 3.0));
  q_d(kIdxV + 2, kIdxQ + 2) =
      t647 - t374 * t641 -
      t373 * (t562 + dt * t343 * (t642 - e_w2 * e_w3 * t397 * (1.0 / 2.0)) *
                         (1.0 / 3.0));
  q_d(kIdxV + 2, kIdxBw + 0) = dt * t343 * t390 * t502 * (-1.0 / 6.0);
  q_d(kIdxV + 2, kIdxBw + 1) = t656;
  q_d(kIdxV + 2, kIdxBw + 2) = dt * t343 * t394 * t503 * (-1.0 / 6.0);
  q_d(kIdxV + 2, kIdxBa + 0) = t659;
  q_d(kIdxV + 2, kIdxBa + 1) = t343 * t372 * t399 * (-1.0 / 2.0);
  q_d(kIdxV + 2, kIdxBa + 2) = t343 * t370 * t400 * (-1.0 / 2.0);
  q_d(kIdxQ + 0, kIdxP + 0) = t564;
  q_d(kIdxQ + 0, kIdxP + 2) = t565;
  q_d(kIdxQ + 0, kIdxV + 0) = t573;
  q_d(kIdxQ + 0, kIdxV + 2) = t580;
  q_d(kIdxQ + 0, kIdxQ + 0) = t374 * (dt - dt * t343 * t432 * (1.0 / 3.0)) +
                              dt * t343 * t502 * (1.0 / 3.0);
  q_d(kIdxQ + 0, kIdxQ + 2) = t652;
  q_d(kIdxQ + 0, kIdxBw + 0) = t343 * t502 * (-1.0 / 2.0);
  q_d(kIdxQ + 1, kIdxP + 0) = dt * t343 * t362 * t373 * (1.0 / 6.0);
  q_d(kIdxQ + 1, kIdxP + 1) = t581;
  q_d(kIdxQ + 1, kIdxP + 2) = dt * t343 * t373 * t397 * (-1.0 / 6.0);
  q_d(kIdxQ + 1, kIdxV + 0) =
      -t371 * (t452 + t623) -
      t374 * (dt * t343 * (t566 - e_w3 * t425) * (1.0 / 3.0) -
  ...
  q_d(kIdxQ + 1, kIdxQ + 0) = -t374 * (e_w3 * t343 * (1.0 / 2.0) -
                                       dt * e_w1 * e_w2 * t343 * (1.0 / 6.0)) -
                              dt * t343 * t373 * t613 * (1.0 / 3.0);
  q_d(kIdxQ + 1, kIdxQ + 1) = t373 * (dt - dt * t343 * t435 * (1.0 / 3.0)) +
                              dt * t343 * t563 * (1.0 / 3.0) +
                              dt * t343 * t371 * t429 * (1.0 / 3.0) +
                              dt * t343 * t374 * t431 * (1.0 / 3.0);
  q_d(kIdxQ + 1, kIdxQ + 2) =
      t655 - t373 * (t614 - dt * e_w2 * e_w3 * t343 * (1.0 / 6.0)) -
      dt * e_w2 * e_w3 * t343 * t374 * (1.0 / 3.0);
  q_d(kIdxQ + 1, kIdxBw + 0) = dt * e_w3 * t343 * t502 * (1.0 / 6.0);
  q_d(kIdxQ + 1, kIdxBw + 1) = t343 * t563 * (-1.0 / 2.0);
  q_d(kIdxQ + 1, kIdxBw + 2) = dt * e_w1 * t343 * t503 * (-1.0 / 6.0);
  ...
  q_d(kIdxQ + 2, kIdxV + 0) =
      ...
      t373 * (t452 - t623);
  q_d(kIdxQ + 2, kIdxV + 1) = -t374 * t631 - t371 * t635 - t373 * (t626 - t627);
  q_d(kIdxQ + 2, kIdxV + 2) =
      t647 - t374 * t641 -
  ...
  q_d(kIdxQ + 2, kIdxQ + 1) = t655 - t373 * (t614 - t653) -
                              dt * e_w2 * e_w3 * t343 * t374 * (1.0 / 3.0);
  q_d(kIdxQ + 2, kIdxQ + 2) = t371 * (dt - dt * t343 * t441 * (1.0 / 3.0)) +
                              dt * t343 * t503 * (1.0 / 3.0) +
                              dt * t343 * t373 * t429 * (1.0 / 3.0) +
                              dt * t343 * t374 * t430 * (1.0 / 3.0);
  q_d(kIdxQ + 2, kIdxBw + 0) = dt * e_w2 * t343 * t502 * (-1.0 / 6.0);
  q_d(kIdxQ + 2, kIdxBw + 1) = t657;
  q_d(kIdxQ + 2, kIdxBw + 2) = t343 * t503 * (-1.0 / 2.0);
  q_d(kIdxBw + 0, kIdxV + 0) = dt * t343 * t362 * t502 * (-1.0 / 6.0);
  q_d(kIdxBw + 0, kIdxV + 2) = dt * t343 * t390 * t502 * (-1.0 / 6.0);
  q_d(kIdxBw + 0, kIdxQ + 0) = t343 * t502 * (-1.0 / 2.0);
  q_d(kIdxBw + 0, kIdxQ + 2) = dt * e_w2 * t343 * t502 * (-1.0 / 6.0);
  q_d(kIdxBw + 0, kIdxBw + 0) = dt * t502;
  q_d(kIdxBw + 1, kIdxV + 0) = dt * t343 * t362 * t563 * (-1.0 / 6.0);
  q_d(kIdxBw + 1, kIdxV + 1) = dt * t343 * t388 * t563 * (-1.0 / 6.0);
  q_d(kIdxBw + 1, kIdxV + 2) = t656;
  q_d(kIdxBw + 1, kIdxQ + 1) = t343 * t563 * (-1.0 / 2.0);
  q_d(kIdxBw + 1, kIdxQ + 2) = t657;
  q_d(kIdxBw + 1, kIdxBw + 1) = dt * t563;
  q_d(kIdxBw + 2, kIdxV + 0) = dt * t343 * t362 * t503 * (-1.0 / 6.0);
  q_d(kIdxBw + 2, kIdxV + 1) = dt * t343 * t381 * t503 * (-1.0 / 6.0);
  q_d(kIdxBw + 2, kIdxV + 2) = dt * t343 * t394 * t503 * (-1.0 / 6.0);
  q_d(kIdxBw + 2, kIdxQ + 1) = dt * e_w1 * t343 * t503 * (-1.0 / 6.0);
  q_d(kIdxBw + 2, kIdxQ + 2) = t343 * t503 * (-1.0 / 2.0);
  q_d(kIdxBw + 2, kIdxBw + 2) = dt * t503;
  q_d(kIdxBa + 0, kIdxP + 0) = dt * t343 * t346 * t398 * (-1.0 / 6.0);
  q_d(kIdxBa + 0, kIdxP + 1) = dt * t343 * t366 * t398 * (-1.0 / 6.0);
  q_d(kIdxBa + 0, kIdxP + 2) = t658;
  q_d(kIdxBa + 0, kIdxV + 0) = t343 * t346 * t398 * (-1.0 / 2.0);
  q_d(kIdxBa + 0, kIdxV + 1) = t343 * t366 * t398 * (-1.0 / 2.0);
  q_d(kIdxBa + 0, kIdxV + 2) = t659;
  q_d(kIdxBa + 0, kIdxBa + 0) = dt * t398;
  q_d(kIdxBa + 1, kIdxP + 0) = t660;
  q_d(kIdxBa + 1, kIdxP + 1) = dt * t343 * t367 * t399 * (-1.0 / 6.0);
  q_d(kIdxBa + 1, kIdxP + 2) = dt * t343 * t372 * t399 * (-1.0 / 6.0);
  q_d(kIdxBa + 1, kIdxV + 0) = t661;
  q_d(kIdxBa + 1, kIdxV + 1) = t343 * t367 * t399 * (-1.0 / 2.0);
  q_d(kIdxBa + 1, kIdxV + 2) = t343 * t372 * t399 * (-1.0 / 2.0);
  q_d(kIdxBa + 1, kIdxBa + 1) = dt * t399;
  q_d(kIdxBa + 2, kIdxP + 0) = dt * t343 * t345 * t400 * (-1.0 / 6.0);
  q_d(kIdxBa + 2, kIdxP + 1) = t662;
  q_d(kIdxBa + 2, kIdxP + 2) = dt * t343 * t370 * t400 * (-1.0 / 6.0);
  q_d(kIdxBa + 2, kIdxV + 0) = t343 * t345 * t400 * (-1.0 / 2.0);
  q_d(kIdxBa + 2, kIdxV + 1) = t663;
  q_d(kIdxBa + 2, kIdxV + 2) = t343 * t370 * t400 * (-1.0 / 2.0);

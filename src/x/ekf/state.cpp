...
State::State(const int n_poses, const int n_features) {
  // Allocate and initialize vision with zero values
  // Note: other states are already initialed at default values by the default
  // constructor.
  p_array_ = Matrix::Zero(n_poses * elementSize::pose, 1);
  q_array_ = Matrix::Zero(n_poses * elementSize::quaternion, 1);
  f_array_ = Matrix::Zero(n_features * elementSize::feature, 1);
  const int n = kSizeCoreErr +
                n_poses * (elementSize::pose + elementSize::smallAngle) +
                n_features * elementSize::feature;
  cov_ = Matrix::Identity(n, n);
  assert(kSizeCoreErr == elementSize::pose + elementSize::velocity +
                             (elementSize::quaternion - 1) +
                             elementSize::bias_a + elementSize::bias_w);
}

State::State(const double time, const unsigned int seq, const Vector3& p,
             const Vector3& v, const Quaternion& q, const Vector3& b_w,
             const Vector3& b_a, const Matrix& p_array, const Matrix& q_array,
             const Matrix& f_array, const Matrix& cov, const Quaternion& q_ic,
             const Vector3& p_ic, const Vector3& w_m, const Vector3& a_m)
    : time_{time},
      seq_{seq},
      p_{p},
      v_{v},
  ...
  assert(p_array.rows() / elementSize::pose ==
             q_array.rows() / elementSize::quaternion ||
         p_array.cols() == 1 || q_array.cols() == 1 ||
         f_array_.rows() % elementSize::feature == 0);
  assert(kSizeCoreErr == elementSize::pose + elementSize::velocity +
                             (elementSize::quaternion - 1) +
                             elementSize::bias_a + elementSize::bias_w);
}

double State::getTime() const { return time_; }

unsigned int State::getSeq() const { return seq_; }

Vector3 State::getPosition() const { return p_; }

Quaternion State::getOrientation() const { return q_; }

Matrix State::getPositionArray() const { return p_array_; }

Matrix State::getOrientationArray() const { return q_array_; }

Matrix State::getFeatureArray() const { return f_array_; }

Quaternion State::getOrientationExtrinsics() const { return q_ic_; }

Vector3 State::getPositionExtrinsics() const { return p_ic_; }

Eigen::VectorXd State::getDynamicStates() const {
  // +1 because not using small angle approx for the quaternion here.
...
Matrix State::getCovariance() const { return cov_; }

Matrix State::getPoseCovariance() const {
#ifdef DEBUG
  if (cov_.cols() == 0 || cov_.rows() == 0) {
    throw std::runtime_error("Call from uninitialized state...");
  }
#endif
...
Matrix& State::getCovarianceRef() { return cov_; }

void State::setTime(const double time) { time_ = time; }
...
void State::setFeatureArray(const Matrix& f_array) { f_array_ = f_array; }

void State::setCovariance(const Matrix& cov) { cov_ = cov; }

void State::setImu(const double time, const unsigned int seq,
...
void State::reset() { time_ = kInvalid; }

int State::nPosesMax() const { return static_cast<int>(p_array_.rows() / 3); }

int State::nFeaturesMax() const {
  return static_cast<int>(f_array_.rows() / 3);
}

int State::nErrorStates() const {
  return kSizeCoreErr + static_cast<int>(p_array_.rows()) +
         static_cast<int>(q_array_.rows() / 4) * 3 +
         static_cast<int>(f_array_.rows());
}
...
void State::correct(const Eigen::VectorXd& correction) {
  ...
  const int n_p_array = static_cast<int>(p_array_.rows());
  const int n_delta_theta_array = n_p_array;
  const int n_f_array = static_cast<int>(f_array_.rows());
  ...
  const Eigen::VectorXd delta_f_array = correction.segment(
  ...
  for (int i = 0; i < n_poses; ++i) {
    // Delta quaternion
    const Vector3 delta_theta_i = delta_theta_array.segment(3 * i, 3);

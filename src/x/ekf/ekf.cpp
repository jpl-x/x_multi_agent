...
Ekf::Ekf(Updater &updater) : updater_{updater} {}
...
void Ekf::set(const Updater &updater, const Vector3 &g,
              const ImuNoise &imu_noise, const int state_buffer_sz,
              const State &default_state, const double a_m_max,
              const unsigned int delta_seq_imu, const double &time_margin_bfr) {
  updater_ = updater;
  state_buffer_ = StateBuffer(state_buffer_sz, default_state, time_margin_bfr);
...
std::optional<State> Ekf::processImu(const double timestamp,
  ...
  if (init_status_ == kStandBy) {
    if (a_m.norm() < a_m_max_) {  // accel spike check
      ...
      BOOST_LOG_TRIVIAL(info) << "Initial state:" << std::endl;
      BOOST_LOG_TRIVIAL(info) << last_state.toString() << std::endl;
      unlock();
      return last_state;
    } else {
      BOOST_LOG_TRIVIAL(warning)
          << "Accelerometer spike detected at seq_id " << seq
  ...
  if (timestamp <= last_state.time_) {
    BOOST_LOG_TRIVIAL(warning)
        << "IMU going back in time. Discarding IMU seq_id " << seq << "."
        << std::endl;
    unlock();
    return std::nullopt;
  }
  ...
  if (seq != last_seq + delta_seq_imu_) {
    BOOST_LOG_TRIVIAL(warning)
        << "Missing IMU messages. Expected seq_id " << last_seq + delta_seq_imu_
        << ", but received seq_id " << seq << "." << std::endl;
  }
  last_seq = seq;
  ...
  if (a_m.norm() < a_m_max_) {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Accelerometer spike detected at seq_id " << seq
        << ". Reading specific force from previous state in buffer."
        << std::endl;
  }
...
#ifdef MULTI_UAV
std::optional<State> Ekf::processOthersMeasurement(const double timestamp) {
  // If the filter is not initialized, do nothing and return invalid state.
  if (init_status_ == kNotInitialized) {
    return std::nullopt;
  }
  ...
  lock();
  const int update_state_idx = state_buffer_.closestIdx(timestamp);
  unlock();
  ...
  // Update state
  updater_.collaborativeUpdate(update_state);

  // Repropagate buffer from update state
  lock();
  const bool update_success =
      repropagateFromStateAtIdx(update_state, update_state_idx);
  unlock();
...
std::optional<State> Ekf::processUpdateMeasurement() {
  // If the filter is not initialized, do nothing and return invalid state.
  if (init_status_ == kNotInitialized) {
    return std::nullopt;
  }
...
bool Ekf::repropagateFromStateAtIdx(const State &state, const int idx) {
  ...
  if (state_buffer_[idx].getTime() == state.getTime()) {
    ...
    BOOST_LOG_TRIVIAL(warning)
        << "Buffer slot at index " << idx
        << " was overwritten by IMU "
           "thread. Update measurement at time "
        << std::setprecision(17) << state.getTime() << "ns"

...
Triangulation::Triangulation(const QuaternionArray& quat_l, Vector3Array pos_l,
                             const unsigned int max_iter, const double term)
    : n_poses_(quat_l.size()),
      positions_(std::move(pos_l)),
      max_iter_(max_iter),
      term_(term) {
...
Triangulation::Triangulation(const x::AttitudeList& attitudes,
                             ...
                             const unsigned int max_iter, const double term)
    : n_poses_(attitudes.size()), max_iter_(max_iter), term_(term) {
  // Initialize pose vectors
  rotations_ = std::vector<Eigen::Matrix3d>(n_poses_, Eigen::Matrix3d());
  ...
  for (size_t i = 0; i < n_poses_; ++i) {
    ...
    Eigen::Vector3d pos(translations[i].tx, translations[i].ty,
                        translations[i].tz);
    positions_[i] = pos;
...
void Triangulation::triangulateDlt(const x::Feature& obs1,
                                   const x::Feature& obs2, const int i1,
                                   const int i2,
                                   Eigen::Vector3d& pt_xyz) const {
  // Feature image coordinates
  cv::Mat obs1_cv = (cv::Mat_<double>(2, 1) << obs1.getX(),
                     obs1.getY());  // Get the normalized coordinates
  cv::Mat obs2_cv = (cv::Mat_<double>(2, 1) << obs2.getX(),
                     obs2.getY());  // Get the normalized coordinates
...
void Triangulation::triangulateGN(const x::Track& track,
  ...
  // the pose vectors
  const int n_obs = static_cast<int>(track.size());
  const int i2 = static_cast<int>(n_poses_) - 1;
  ...
  const cv::Mat pt_w_h =
      (cv::Mat_<double>(4, 1) << pt_xyz(0), pt_xyz(1), pt_xyz(2), 1);
  ...
  while (r_norm_last - r_norm > term_) {
    ...
    Eigen::VectorXd r = Eigen::VectorXd::Zero(static_cast<int>(n_meas));
    Eigen::MatrixXd j = Eigen::MatrixXd::Zero(static_cast<int>(n_meas), 3);
    ...
    for (int i = i1; i <= i2; i++) {
      ...
      Eigen::Vector2d h_meas(track[i_trk].getX(), track[i_trk].getY());

      // Predicted measurement
      Eigen::Vector3d h_i =
          delta_rot_i2 * Eigen::Vector3d(alpha, beta, 1) + rho * delta_pos_i2;
      ...
      // Jacobians
      const Eigen::Vector3d j_alpha = delta_rot_i2 * u_x;
      const Eigen::Vector3d j_beta = delta_rot_i2 * u_y;
      const Eigen::Vector3d& j_rho = delta_pos_i2;

      Eigen::Matrix3d j0;
      j0 << j_alpha, j_beta, j_rho;

      Eigen::MatrixXd j1(2, 3);
      j1 << -1.0 / h_i(2), 0.0, h_i(0) / std::pow(h_i(2), 2), 0.0, -1 / h_i(2),
          h_i(1) / std::pow(h_i(2), 2);
    ...
    const Eigen::Vector3d delta =
        (j.transpose() * j).inverse() * j.transpose() * r;
    alpha = alpha - delta(0);
    beta = beta - delta(1);
    rho = rho - delta(2);
...
void Triangulation::pose2proj(const Eigen::Matrix3d& rot,
                              const Eigen::Vector3d& pos, cv::Mat& proj) const {

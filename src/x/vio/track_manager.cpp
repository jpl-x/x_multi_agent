...
TrackList TrackManager::getShortMsckfTracks() const {
  return msckf_trks_short_n_;
}
...
TrackList TrackManager::getOppTracks() {
...
#ifdef MULTI_UAV
FeatureList TrackManager::getOppFeatures() const {
...
TrackList TrackManager::getMostPromisingTracks(int k) {
  TrackList tmp_list = slam_trks_;
  std::sort(tmp_list.begin(), tmp_list.end(),
            [](const Track &a, const Track &b) {
              return a.back().getFastScore() > b.back().getFastScore();
            });
  if (tmp_list.size() >= static_cast<size_t>(k)) {
    return TrackList(tmp_list.begin(), tmp_list.begin() + k);
  }
  return tmp_list;
}

void TrackManager::manageTracks(MatchList &matches, const AttitudeList cam_rots,
  ...
  slam_trks_.insert(slam_trks_.end(), new_slam_trks_.begin(),
                    new_slam_trks_.end());
  new_slam_trks_.clear();
  ...
  while (t < slam_trks_.size()) {
    ...
    while (m < matches.size()) {
      // Check if the last feature of the persistent track is the previous
      // match
      if (slam_trks_[t].back() == matches[m].previous) {
        ...
        const unsigned int bin_nbr =
            matches[m].current.getTileRow() * img.getNTilesW() +
            matches[m].current.getTileCol();
    ...
    if (isTrackLost) {
      // Add its index to lost tracks so it is deleted from the filter state
  ...
  while (m < matches.size())  // Matches are in descending FAST order
  {
    ...
    while (t < previous_opp_trks.size()) {
      // Check if the last feature of the opportunistic track is the previous
      // match
      if (previous_opp_trks[t].back() == matches[m].previous) {
        isTrackLost = false;  // we found the track, so it is not lost
        // Add the matched feature to the track
...
// These tracks refer to the previus state, cannot be updated as MSCKF since
// they will be considered with the wrong camera position, a separate list
// must be initialized from the lost msckf tracks
//
#ifdef MULTI_UAV
  count_multi_msckf_ = 0;
  AttitudeList cam_rots_short = cam_rots;
  cam_rots_short.pop_back();
  for (auto &dead_track : previous_opp_trks) {
    if (dead_track.size() < 3) {
      continue;
    }

    for (size_t o = 0; o < opp_ids_->size(); o++) {
      if (dead_track.getId() == opp_ids_->at(o)) {
        // Normalize track (and crop it if it's longer than the attitude
        // list)
  ...
  std::sort(opp_trks_.begin(), opp_trks_.end(),
            [](const Track &a, const Track &b) { return a.size() > b.size(); });
  t = 0;
  while (t < opp_trks_.size()) {
...
#ifdef MULTI_UAV
    if (false && opp_ids_ != nullptr) {
      bool match = false;
      for (size_t o = 0; o < opp_ids_->size(); o++) {
        if (opp_trks_.at(t).getId() == opp_ids_->at(o)) {
    ...
    if (opp_trks_[t].size() > min_track_length - 1) {
      ...
      if ((slam_trks_.size() + new_slam_trks_.size()) < n_slam_features_max) {
        ...
        opp_trks_.erase(opp_trks_.begin() + t);
        bin_track_idx[bin_nbr].push_back(slam_trks_.size() +
                                         new_slam_trks_.size() - 1);
              ...
              // spread out their distribution
      {
        ...
        if (bin_track_idx[idx_bin_max_per].size() >
          ...
          if (bin_track_idx[idx_bin_max_per].back() >= slam_trks_.size()) {
            new_slam_trks_.erase(new_slam_trks_.begin() +
                                 bin_track_idx[idx_bin_max_per].back() -
                                 slam_trks_.size());
          } else {
            // TODO(jeff): This persistent track should still be processed and
            // be removed after
            // the update
            lost_slam_idxs_.push_back(
                bin_track_idx_per[idx_bin_max_per].back());
            bin_track_idx_per[idx_bin_max_per].pop_back();
            slam_trks_.erase(slam_trks_.begin() +
                             bin_track_idx[idx_bin_max_per].back());
          }
  ...
  // SLAM in Update.
  new_slam_trks_.clear();
  new_slam_trks_.insert(new_slam_trks_.end(), new_slam_msckf_trks.begin(),
                        new_slam_msckf_trks.end());
  new_slam_trks_.insert(new_slam_trks_.end(), new_slam_std_trks.begin(),
 ...
 */
std::vector<int> TrackManager::featureTriangleAtPoint(const Feature &lrf_img_pt,
                                                      TiledImage &img) const {
  /*******************************************
   * Delaunay triangulation on SLAM features *
   *******************************************/

  // Rectangle to be used with Subdiv2D (account +/- 0.5 offset on coordinates
  // at edges)
   ...
   ******************************************/
  // Note: The first 4 points of the subdivision correspond to the edges of
  // the image. If they are one of the triangle's vertex, that triangle is not
  // valid.

  std::vector<int> tr_feat_ids = {-1, -1, -1};

  // If lrf falls inside or on the edge of a triangle
  if (status == cv::Subdiv2D::PTLOC_INSIDE ||
      status == cv::Subdiv2D::PTLOC_ON_EDGE) {
  ...
  for (unsigned int i = 0; i < 3; i++) {
    // The first 4 points of the subdivision correspond to the edges of the
    // image. If they are one of the triangle's vertex, that triangle is not
    // valid.
    if (tr_feat_ids[i] > -1) {
...
// Draw delaunay triangles
void TrackManager::draw_delaunay(cv::Mat &img, cv::Subdiv2D &subdiv,
...
bool TrackManager::checkBaseline(const Track &track,
  ...
  for (int i = i_first; i <= i_last; ++i) {
    ...
    const Eigen::Quaterniond Cn_q_Ci =
        Ci_q_G.normalized().conjugate() * Cn_q_G.normalized();
    const Eigen::Quaterniond q_ray_Ci(0.0, track[i_trk].getX(),
                                      track[i_trk].getY(), 1.0);
  ...
  if (delta_feat_x > min_baseline_x_n_ || delta_feat_y > min_baseline_y_n_)
    return true;
  else
    return false;
}

void TrackManager::plotFeatures(TiledImage &img, const int min_track_length) {
  ...
  static const cv::Scalar orange(0, 204, 249);
  static const cv::Scalar blue(249, 144, 24);
  static const cv::Scalar red(255, 0, 0);
  static const cv::Scalar purple(100, 0, 100);
...
#ifdef MULTI_UAV
  for (const auto &t : collab_opp_featrues_print_) {
    img.plotFeature(t, red);
  }
  collab_opp_featrues_print_.clear();
#endif
  for (const auto &t : msckf_featrues_print_) {
    img.plotFeature(t, purple);
  }
  msckf_featrues_print_.clear();

  std::string slam_str = std::string("SLAM: ") +
                         std::to_string(n_slam + n_new_slam) +
                         std::string(" - ");
  std::string opp_str = std::string("Opp: ") + std::to_string(count_opp);

  std::string msckf_str = std::string(" - MSCKF: ") +
                          std::to_string(msckf_trks_n_.size()) +
                          std::string(" - Short MSCKF: ") +

...
VioUpdater::VioUpdater(Tracker &tracker, StateManager &state_manager,
...
void VioUpdater::preProcess(const State &state) {
  // Check if matches are already available, else find them in the current
  // available image
  if (measurement_.matches.empty()) {
    match_img_ = measurement_.image;

    // Track features
    tracker_.track(measurement_.image, measurement_.timestamp,
                   measurement_.seq);
  ...
  OppIDListPtr opp_ids = tracker_.getOppIds();
...
bool VioUpdater::preUpdateShortMsckf() {
  ...
  slam_matches_ = tracker_.getSlamMatches();
#endif

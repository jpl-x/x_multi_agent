...
void StateManager::manage(State &state,
    ...
    while (i) {
        ...
        const unsigned int n1 =
                n_features_ - idx - 1; // number of coming features after it
        ...
        const unsigned int idx0 = kSizeClone + n_poses_max_ * 6 + idx * 3;
        ...
        Matrix cols_after = cov.block(0, idx1, n, dim0);

        // Remove the rows corresponding to the removed feature in them
        cols_after.block(idx0, 0, dim0, dim0) =
                cols_after.block(idx1, 0, dim0, dim0);
        ...
        Matrix rows_after = cov.block(idx1, 0, dim0, idx0);

        // Shift cols_after and rows_after one index up in the covariance matrix
        cov.block(0, idx0, n, dim0) = cols_after;
        cov.block(idx0, 0, dim0, idx0) = rows_after;
...
void StateManager::initMsckfSlamFeatures(State &state,
    ...
    const Matrix new_features =
            init_mats.features - H2_inv_H1 * correction + H2_inv * init_mats.r1;
...
void StateManager::initStandardSlamFeatures(State &state,
    ...
    Matrix P_diag_new = var_img * Matrix::Identity(n_new_states, n_new_states);
...
void StateManager::addFeatureStates(State &state, const Matrix &new_features,
                                    const Matrix &cov, const Matrix &cross) {
    ...
    assert(n_features_ < static_cast<size_t>(n_features_max_));
    ...
    const size_t n_states = kSizeClone + n_poses_max_ * 6 + n_features_ * 3;
...
/** \brief Returns the cartesian coordinates of SLAM features for the input
 * state
 */
std::vector<Eigen::Vector3d>
StateManager::computeSLAMCartesianFeaturesForState(const State &state) const {
    ...
    for (unsigned int i = 0; i < n_features_; i++) {
        ...
        Eigen::Vector3d G_p_fi = 1 / (rho) * Ci_q_G.normalized().toRotationMatrix() *
                                 Eigen::Vector3d(alpha, beta, 1) +
                                 G_p_Ci;
...
void StateManager::augmentCovariance(const State &state, const int pos,
                                     Matrix &covariance) {
    Matrix jacobianCamPoseWrtStates;
    if (not stateHasBeenFilledBefore_) {
        jacobianCamPoseWrtStates =
                Matrix::Zero(kSizeClone + n_poses_max_ * 6 + n_features_max_ * 3,
                       kSizeClone + n_poses_max_ * 6 + n_features_max_ * 3);
    } else {
        jacobianCamPoseWrtStates =
                Matrix::Identity(kSizeClone + n_poses_max_ * 6 + n_features_max_ * 3,
                           kSizeClone + n_poses_max_ * 6 + n_features_max_ * 3);
    }

    // COVARIANCE AUGMENTATION
    // Unchanged: previous Core and cam position
    jacobianCamPoseWrtStates.block(0, 0, kSizeClone + (pos + 1) * 3,
                                   kSizeClone + (pos + 1) * 3) =
            Matrix::Identity(kSizeClone + (pos + 1) * 3,
                         kSizeClone + (pos + 1) * 3);

    // Unchanged: previous cam attitude
    jacobianCamPoseWrtStates.block(kSizeClone + n_poses_max_ * 3,
                                   kSizeClone + n_poses_max_ * 3, (pos + 1) * 3,
                                   (pos + 1) * 3) =
            Matrix::Identity((pos + 1) * 3, (pos + 1) * 3);

    // Unchanged: features
    jacobianCamPoseWrtStates.block(kSizeClone + n_poses_max_ * 6,
                                   kSizeClone + n_poses_max_ * 6,
                                   n_features_ * 3, n_features_ * 3) =
            Matrix::Identity(n_features_ * 3, n_features_ * 3);

    // cam_position::G_p_I
    // Derivative of camera position error wrt imu position error = I3x3
    jacobianCamPoseWrtStates.block(kSizeClone + pos * 3, 0, 3, 3) =
            Matrix::Identity(3, 3);

    // Derivative of camera position error wrt imu orientation error = - C(qI2G) *
    // [p_ic x]
    Vector3 pci = state.getPositionExtrinsics();
    jacobianCamPoseWrtStates.block(kSizeClone + pos * 3, 6, 3, 3) =
            -state.getOrientation().normalized().toRotationMatrix() *
            pci.toCrossMatrix();

    // Derivative of camera attitude error wrt imu orientation error = C(qI2C)
    jacobianCamPoseWrtStates.block(kSizeClone + n_poses_max_ * 3 + pos * 3, 6,
                                   3, 3) = state.getOrientationExtrinsics()
            .conjugate()
            .normalized()
            .toRotationMatrix();
    ...
    P_current.block(kSizeClone + pos * 3, 0, 3, P_current.cols()) =
            Matrix::Zero(3, P_current.cols());
    // Delete rows associated with attitude
    P_current.block(kSizeClone + n_poses_max_ * 3 + pos * 3, 0, 3,
                    P_current.cols()) = Matrix::Zero(3, P_current.cols());

    // Delete cols associated with position
    P_current.block(0, kSizeClone + pos * 3, P_current.rows(), 3) =
            Matrix::Zero(P_current.cols(), 3);
    // Delete cols associated with attitude
    P_current.block(0, kSizeClone + n_poses_max_ * 3 + pos * 3,
                    P_current.rows(), 3) = Matrix::Zero(P_current.cols(), 3);
    ...
    P_current = jacobianCamPoseWrtStates * P_current *
...
void StateManager::reparametrizeFeatures(Eigen::VectorXd const &atts_old,
    ...
    const size_t n = kSizeClone + n_poses_max_ * 6 + n_features_max_ * 3;
    ...
    for (unsigned int i = 0; i < idx_to_chg.size(); i++) {
        ...
        const Vector3 new_params =
                ...
                (-G_p_Ci_new1 + G_p_Ci_old +
                 1 / rho_old * Ci_q_G_old.normalized().toRotationMatrix() *
        ...
        // Old anchor attitude
        Eigen::Vector3d skew_vector = Eigen::Vector3d(alpha_old, beta_old, 1);
        const StateManager::Jacobian J_anchor_att_old =
                -1 / rho_old * Ci_q_G_new.normalized().toRotationMatrix().transpose() *
                Ci_q_G_old.normalized().toRotationMatrix() *
                x::Skew(skew_vector(0), skew_vector(1), skew_vector(2)).matrix;

        // New anchor attitude
        skew_vector = Ci_q_G_new.normalized().toRotationMatrix().transpose() *
                      (-G_p_Ci_new1 + G_p_Ci_old +
                       1 / rho_old * Ci_q_G_old.normalized().toRotationMatrix() *
                       Eigen::Vector3d(alpha_old, beta_old, 1));
        ...
        const StateManager::Jacobian J_anchor_feat_old =
                1 / rho_old * Ci_q_G_new.normalized().toRotationMatrix().transpose() *
                Ci_q_G_old.normalized().toRotationMatrix() * mat;
        ...
        A_j.block<jac_rows, jac_cols>(0, kSizeCoreErr + col) = J_anchor_pos_new;

        col += n_poses_max_ * jac_cols;
        A_j.block<jac_rows, jac_cols>(0, kSizeCoreErr + col) = J_anchor_att_new;

        col = 0;
        A_j.block<jac_rows, jac_cols>(0, kSizeCoreErr + col) = J_anchor_pos_old;

        col += n_poses_max_ * jac_cols;
        A_j.block<jac_rows, jac_cols>(0, kSizeCoreErr + col) = J_anchor_att_old;
...
void StateManager::slideWindow(Eigen::VectorXd &atts, Eigen::VectorXd &poss,
                               Matrix &covariance) {
    ...
    left_mult.block(0, 0, kSizeClone, kSizeClone) =
            Matrix::Identity(kSizeClone, kSizeClone);

    if (n_features_max_) {
        left_mult.block(kSizeClone + n_poses_max_ * 6,
                      kSizeClone + n_poses_max_ * 6, n_features_max_ * 3,
                        n_features_max_ * 3) =
    ...
    left_mult.block(kSizeClone, kSizeClone + 3, (n_poses_max_ - 1) * 3,
                    (n_poses_max_ - 1) * 3) =
            Matrix::Identity((n_poses_max_ - 1) * 3, (n_poses_max_ - 1) * 3);

    left_mult.block(kSizeClone + n_poses_max_ * 3,
                    kSizeClone + 3 + n_poses_max_ * 3, (n_poses_max_ - 1) * 3,
                    (n_poses_max_ - 1) * 3) =
            Matrix::Identity((n_poses_max_ - 1) * 3, (n_poses_max_ - 1) * 3);

    right_mult.block(kSizeClone + 3, kSizeClone, (n_poses_max_ - 1) * 3,
                     (n_poses_max_ - 1) * 3) =
            Matrix::Identity((n_poses_max_ - 1) * 3, (n_poses_max_ - 1) * 3);

    right_mult.block(kSizeClone + 3 + n_poses_max_ * 3,
                     kSizeClone + n_poses_max_ * 3, (n_poses_max_ - 1) * 3,
                     (n_poses_max_ - 1) * 3) =
            Matrix::Identity((n_poses_max_ - 1) * 3, (n_poses_max_ - 1) * 3);
...
StateManager::convertCameraAttitudesToList(const State &state,
    ...
    const int start_idx(n_poses_ - size_out);
    for (int i = start_idx; i < n_poses_; i++) {
        // ax ay az aw
        const x::Attitude attitude(
                orientation_array(4 * i, 0), orientation_array(4 * i + 1, 0),
                orientation_array(4 * i + 2, 0), orientation_array(4 * i + 3, 0));
...
x::TranslationList
StateManager::convertCameraPositionsToList(const State &state) const {
    ...
    for (int i = 0; i < n_poses_; i++) {
        const x::Translation translation(
                positions(3 * i, 0), positions(3 * i + 1, 0), positions(3 * i + 2, 0));
